...
tau_str = ["0.500000","0.600000","0.800000", "1.200000","1.250000","1.400000", "1.500000","1.750000"] 
tau_num = []
for i in range(len(tau_str)): tau_num.append(float(tau_str[i]))

tau_len = len(tau_str)
names = ["Gauss Method", "Overrelaxation Method with tau=0.500000", "Overrelaxation Method with tau=1.250000", "Overrelaxation Method with tau=1.500000", "Overrelaxation Method with tau=1.750000", "Seidel Method",  "Jacobi Method"]
len = 7
...
for i in range(len):
	...
	if (i > 0):
		for k in range(mu_len):
			n = 10**k
			print "full:", "slow",names[i], ": size =", n
	...
	#graph only with mu and fast methods
	ax[i] = plt.subplot()
	for k in range(mu_len):
		n = 10**k
		print "only fast:", names[i], ": size = ", n
		#data[i].drop(data[i][((data[i].t-f(data[i].n*1.0, coeff[0], coeff[1], coeff[2])) > data[i].t*0.2)].index, axis=0, inplace = True)
		data[i][(data[i].mu==n)].plot(x='n', y = 't', kind='scatter',label = "mu = "+str(n),color=colors[k+1], ax=ax[i], s = 5, marker = 'o') #  & (data[i].bad != True)
		ax[i].plot(x[k+i*mu_len],y[k+i*mu_len],color=colors[k+1], linewidth = 0.5)
	...
	plt.title(names[i] + r"\n$\log_{10}$(number of conditionality)")
	#if (fit == 2): plt.text(data[i].n.max()/2, data[i].t.max()/3, "T = " + str(coeff[0])+r"$\cdot x^2$ + " + str(coeff[1]) + r"$\cdot x$")
	plt.savefig("temp_graphs/mu/"+names[i]+" mu.png", dpi=500)
	plt.clf()


print "plot all methods"
com = plt.subplot();
for i in range(len-2):
	k = i+1;
	data[k].plot(x='n', y = 't',kind = 'scatter', label = names[k], color=colors[k], ax=com, s = 5)
	com.plot(x[k*mu_len], y[k*mu_len], label = names[k]+"line", linewidth = 0.5, color=colors[k])
plt.legend(loc=2, fontsize='xx-small')
plt.grid(True)
plt.xlabel('size of matrix')
plt.ylabel('T, c')
...
#work with tau
fit = 2
print "plot overrelaxation methods"
data_tau = dict()
coeff_tau = []
